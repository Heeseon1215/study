...
## 241219
**portfolio**
- ben &  jerry's build 후 업로드
  - 웹폰트 경로와 이미지 경로 문제 → 해결 완료
- 상세 페이지 퍼블리싱 완료
- 메인 페이지 퍼블리싱 수정 시작
...
## 241217
**portfolio**
- ben &  jerry's 스크립트 살짝 진행
- ben &  jerry's React작업 시작

## 241216
**portfolio**
- ben &  jerry's 모바일 페이지 css 완료
  - 메뉴 클릭하면 왜 오픈이 안될까?
  - 스크롤 내려갈때 이미지도 함께 내려가는 방법 찾아보기

## 241213
**portfolio**
- ben &  jerry's 디자인 완료
- ben &  jerry's 모바일 페이지 퍼블리싱 시작
...
## 241211
**React**
- 리액트에서 `build` 후 업로드 하는 방법에 대해 배웠다
...
## 241210
**React**
- 버전 업데이트 후 오류를 해결하는 방법에 대해 알게되었다
- 리액트에서 `swiper`를 사용하는 방법에 대해 배웠다
  
**portfolio**
- SONY 웹페이지 gsap 작업
  - 옆으로 움직임 Ok. 근데 이제 움직이고 나면 vue 아이콘만 남는다. . . .
  - 아이콘만 남는 이유는 이미지를 따로 움직여서. 둘이 함께 움직이게 하니 괜찮아졌다. 

## 241209
**React**
- 리액트에서 여러 `className` 사용 하는 방법에 대해 배웠다
- 리액트의 `전역`에 대해 배웠다
- `Scss 변수 선언` 방법에 대해 배웠다.
  
**portfolio**
- SONY 웹페이지 퍼블리싱
  - gsap 작업 시작
    - 스크롤 내렸을때 vue 아이콘이 작아졌다가 옆으로 움직인 후 확대하는 부분에서 막힘 .
    - 작아지는 부분까지는 괜찮은데 왜 그 후에 옆으로 움직이고 확대가 안될까?


## 241206
**React**
- css 모듈에 대해 배웠다
- `dataList` 외부 데이터 처리 방법에 대해 배웠다
...
**React**
- `state`에 대해 배웠다
- 리액트에서 `파비콘` 연결 방법에 대해 배웠다
- `set` 함수에 대해 배웠다

## 241204
**React**
- `react` 설치를 하였다
- `jsx` 문서에 대해 배우고 사용 방법에 대해 배웠다
...
## 241203
**React**
- `kakao api`를 불러오는 방법에 대해 배웠다.
- `map`과 `spread 연산자`,  `filter`,  `삼항연산자`,  `단락회로평가`, `구조분해할당`에 대해 배웠다
...
- `json`에 대해 배웠다.
- `fetch`에 대해 배웠다.
- `async+await`에 대해 배우고 예제를 진행하였다.
- `jQuery Ajax`에 대해 배웠다.
